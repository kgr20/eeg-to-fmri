...
# ---------------------------------------------------------------------------------------
# Example: Define train, val, and test splits
# ---------------------------------------------------------------------------------------
# Let's say we have the following subject IDs in the data root:
# ['32', '43', '44', '45', '46'] ... (just an example)
# We'll use '43', '44' for testing, '32' for validation, and the rest for training
test_list = ['43', '44']
val_list = ['32']   # pick one subject for validation
train_list = [
    Path(indv).stem
    for indv in os.listdir(args.data_root)
    if Path(indv).stem not in test_list + val_list 
       and (Path(args.data_root) / indv).is_file()
]

print("Training Individuals:", sorted(train_list))
print("Validation Individuals:", val_list)
...
eeg_val, fmri_val = load_h5_from_list(args.data_root, val_list)

print("Loading test data...")
eeg_test, fmri_test = load_h5_from_list(args.data_root, test_list)

print("[Shapes before transpose]")
print(" Train EEG:", eeg_train.shape, " Train fMRI:", fmri_train.shape)
print(" Val   EEG:", eeg_val.shape,   " Val   fMRI:", fmri_val.shape)
print(" Test  EEG:", eeg_test.shape,  " Test  fMRI:", fmri_test.shape)

# ---------------------------------------------------------------------------------------
# Transpose to match PyTorch [N, C, H, W]
# ---------------------------------------------------------------------------------------
def transpose_data(eeg, fmri):
    eeg = eeg.transpose(0, 3, 1, 2)   # (B, 10, 64, 269) or shape you need
    fmri = fmri.transpose(0, 3, 1, 2) # (B, 30, 64, 64)
    return eeg, fmri

eeg_train, fmri_train = transpose_data(eeg_train, fmri_train)
eeg_val, fmri_val = transpose_data(eeg_val, fmri_val)
eeg_test, fmri_test = transpose_data(eeg_test, fmri_test)
...
# Normalize data
# ---------------------------------------------------------------------------------------
def normalize_data(data: np.ndarray, scale_range: Tuple=(0,1)):
    """
...
eeg_train = normalize_data(eeg_train)
fmri_train = normalize_data(fmri_train)
eeg_val = normalize_data(eeg_val)
fmri_val = normalize_data(fmri_val)
eeg_test = normalize_data(eeg_test)
fmri_test = normalize_data(fmri_test)
...
val_dataset = EEGfMRIDataset(
    torch.tensor(eeg_val, dtype=torch.float32),
    torch.tensor(fmri_val, dtype=torch.float32)
)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        # Example: reflect-pad from w=269 -> w=288 to get shape multiples of 2^4
        desired_w = 288
        if w < desired_w:
            pad_amt = desired_w - w
            x = F.pad(x, (0, pad_amt, 0, 0), mode='reflect')
        x = self.encoder(x)
        return x  # (B,512,4,18) after these strides
...
def combined_mse_ssim_loss(pred, target, ssim_module, alpha=0.5):
    """
    combined loss = alpha * MSE + (1 - alpha) * (1 - SSIM)
    """
...
# ---------------------------------------------------------------------------------------
# Diffusion Scheduler (Increased Timesteps)
# ---------------------------------------------------------------------------------------
# Instead of 10, let's try 100 (or more).
...
for epoch in pbar:
    ...
    for inputs, labels in train_loader:
        ...
        # minimal noise injection from the DDPMScheduler
    ...
    with torch.no_grad():
        for inputs, labels in val_loader:
            ...
            outputs = model(noisy_inputs, timesteps)
            val_ssim += calculate_ssim(outputs, labels).item()
            val_psnr += calculate_psnr(outputs, labels)

    val_ssim /= len(val_loader)
    val_psnr /= len(val_loader)

    # ------------------------
    # Test Phase (Optional at each epoch)
    # ------------------------
    # If we want to track test metrics every epoch (some might prefer only at final):
    test_ssim = 0.0
    test_psnr = 0.0
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            outputs = model(noisy_inputs, timesteps)
            test_ssim += calculate_ssim(outputs, labels).item()
            test_psnr += calculate_psnr(outputs, labels)

    test_ssim /= len(test_loader)
    test_psnr /= len(test_loader)

    # Visualization on last batch of test data (just to log an image)
    ...
    if val_ssim > best_ssim:
        ...
        best_save_path = os.path.join(
            exp_dir, f"epoch{epoch}_valssim_{best_ssim:.4f}_valpsnr_{best_psnr:.2f}.pth"
        )
        torch.save(model.state_dict(), best_save_path)

    pbar.set_description(
        f"Epoch {epoch} - Loss: {epoch_loss:.4f} | Val SSIM: {val_ssim:.3f} | Val PSNR: {val_psnr:.3f}"
    )
