# this works, but the output is bad and not super clear. SSIM of 0.6 and climbing after 83 epochs.  
# dataset_01_run_20240730-065927
# 0.85 SSIM on the same individual
...
class CustomUNetDiffusion(nn.Module):
    ...
    def center_crop(self, tensor, target_size):
        ...
        mse_loss = F.mse_loss(cropped_tensor, tensor[:, :, :th, :tw])
        return cropped_tensor, mse_loss.item()
    ...
    def forward(self, x):
        ...
        # Decoder
        x, mse_loss = self.center_crop(x_noisy, x6.shape[2:])
        x = self.decoder6(x + x6)
        # print(f"After decoder6: {x.shape}")

        x, mse_loss = self.center_crop(x, x5.shape[2:])
        x = self.decoder5(x + x5)
        # print(f"After decoder5: {x.shape}")

        x, mse_loss = self.center_crop(x, x4.shape[2:])
        x = self.decoder4(x + x4)
        # print(f"After decoder4: {x.shape}")

        x, mse_loss = self.center_crop(x, x3.shape[2:])
        x = self.decoder3(x + x3)
        # print(f"After decoder3: {x.shape}")

        x, mse_loss = self.center_crop(x, x2.shape[2:])
        x = self.decoder2(x + x2)
        # print(f"After decoder2: {x.shape}")

        x, mse_loss = self.center_crop(x, x1.shape[2:])
