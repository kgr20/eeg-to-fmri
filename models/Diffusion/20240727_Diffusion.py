...
class SimpleConvModel(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1)  # Downsample
        self.upsample = nn.Upsample(size=(127, 127), mode='bilinear', align_corners=True)
        self.conv2 = nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1) # Downsample
        self.conv3 = nn.Conv2d(128, 64, kernel_size=1)  # 1x1 conv to reduce depth
...
for epoch in pbar:
    ...
    for inputs, labels in train_loader:
        ...
        # Calculate SSIM loss
        ssim_loss = 1 - ssim_metric(outputs, labels)
        loss = ssim_loss
        loss.backward()
