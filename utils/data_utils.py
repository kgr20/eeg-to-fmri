...
n_individuals_train_NEW=1
n_individuals_test_NEW=0
...
def get_data(individuals, raw_eeg=False, raw_eeg_resample=False, eeg_resample=2.160, bold_shift=3, n_partitions=16, by_partitions=True, partition_length=None, n_voxels=None, TR=2.160, f_resample=2, mutate_bands=False, eeg_limit=False, eeg_f_limit_h=134, eeg_f_limit_l=0, fmri_resolution_factor=5, standardize_eeg=True, standardize_fmri=True, ind_volume_fit=True, iqr_outlier=True, dataset="01"):
    ...
    print(f"data_utils.py: len(individual_imgs): {len(individuals_imgs)}")
    # data_utils.py: individual_imgs: [<nibabel.nifti1.Nifti1Image object at 0x7f86b3ccde50>]. Length is 1 for one individual. This should be the 3D volume
    ...
    for i in range(len(individuals_imgs)):
        ...
        for volume in range(individuals_imgs[i].shape[-1]):
            ...
            # fmri_volumes.shape = (297, 64, 64, 30)
            # scaled_volume.shape: (1, 64, 64, 30)
            j += 1
            # this is a counter which is used for assigning the fmri_volume[#] to the correct scaled volume
    ...
    for individual in individuals:
        ...
        # individuals_eegs.shape: (0, 64, 134)
        ...
        # Individual_eegs.shape: (297, 64, 134)
        # individuals_imgs.shape: (297, 64, 64, 30)
        # len(fmri_scalers): 0
...
# Missing understanding still for this one ~~~
def create_eeg_bold_pairs(eeg, fmri, raw_eeg=False, fs_sample_eeg=250, fs_sample_fmri=2, interval_eeg=2, n_volumes=300, n_individuals=10, instances_per_individual=16):
    if(raw_eeg):
        ...
        # This is where we add another dimension (interval_eeg, which is 10)
        # n_individuals 0 (and also 1), eeg.shape[1:] (64, 134), n_volumes: 297, interval_eeg: 10
    ...
    for individual in range(n_individuals): 
        for index_volume in range(individual*(n_volumes), individual*(n_volumes)+n_volumes-interval_eeg):#the last observation is missing?
            if(raw_eeg):
                ...
                if(np.transpose(eeg[index_volume:index_volume+interval_eeg], (1,2,0)).shape[-1]!=interval_eeg): # Before transposition: (500, 64, 30) (500 time points, 64 channels, 30 frequency bins). After transposition: (64, 30, 500). 
                    continue
                x_eeg[index_volume-individual*interval_eeg] = np.transpose(eeg[index_volume:index_volume+interval_eeg], (1,2,0))
                # individual = 0 when printed with only one person
            x_fmri[index_volume-individual*interval_eeg] = fmri[index_volume+interval_eeg]

        print (f"data_utils.py: x_eeg.shape, x_fmri.shape: {x_eeg.shape, x_fmri.shape}. (individual {individual})")
    return x_eeg, x_fmri
